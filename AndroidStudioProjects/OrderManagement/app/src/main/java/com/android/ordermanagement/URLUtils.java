package com.android.ordermanagement;

/**
 * Created by challa on 17/02/17.
 */

public class URLUtils {
    public static final String LOGIN = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/GetUserDetails";

    public static final String SALES_EXE = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/SalesExeServices";

    public static final String SALES_DETAILS_EXE = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/GetSalesmenAreaDetails";

    public static final String GET_CUSTOMERS_LIST = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/So_MasterDetails";

    public static final String ADD_CUSTOMER = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/DistCustomerCreation";

    public static final String GET_PRODUCTS_LIST = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/ItemDetails";

    public static final String POST_ORDER = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/SaleOrder";

    public static final String GET_ORDERS = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/GetSaleOrderDetails";

    public static final String INVOICE = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/InvoiceCreation";

    public static final String SALES_DASHBOARD = "http://loginwebservice.laksanasoft.com/LoginWebService.asmx/SalesMenDashboardSales";

}
